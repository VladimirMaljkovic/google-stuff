...
def solve_using_ford_fulkerson(normalized_network):
    pass
...
if __name__ == '__main__':
    ...
    # solve_using_ford_fulkerson(normalize(entrances, exits, path))

    g = Graph(normalize(entrances, exits, path))
    source = 0
    sink = len(path) + 2

    print(f'Max possible flow is {g.FordFulkerson(source, sink)}')
